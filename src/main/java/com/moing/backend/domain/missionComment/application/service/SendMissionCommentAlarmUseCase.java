...
import static com.moing.backend.global.config.fcm.constant.NewCommentUploadMessage.*;
...
public class SendMissionCommentAlarmUseCase {
	...
	public void sendCommentAlarm(BaseMissionServiceResponse response, MissionComment comment) {
		...
		Optional<List<NewUploadInfo>> newUploadInfos = missionCommentGetService.getNewUploadInfo(member.getMemberId(),
			missionArchive.getId());
	...
	private void sendMissionWriter(MissionArchive missionArchive, Mission mission,
		...
		if (checkMemberWriter(receiver, member, newUploadInfos)) {
			eventPublisher.publishEvent(new SingleFcmEvent(receiver, title, body,
				createIdInfo(team.getTeamId(), mission.getId(), missionArchive.getId()), team.getName(),
				AlarmType.COMMENT, PagePath.MISSION_PATH.getValue(), receiver.isCommentPush()));
		}
	}
	...
	private void sendMissionCommentWriter(Mission mission, MissionArchive missionArchive,
		...
		eventPublisher.publishEvent(new MultiFcmEvent(title, body, memberIdAndTokensByPush, memberIdAndTokensBySave,
			createIdInfo(team.getTeamId(), mission.getId(), missionArchive.getId()), team.getName(), AlarmType.COMMENT,
			PagePath.MISSION_PATH.getValue()));
	}

	private boolean checkMemberWriter(Member missionWriter, Member commentWriter,
		Optional<List<NewUploadInfo>> newUploadInfos) {
