...
import static com.moing.backend.domain.boardComment.presentattion.constant.BoardCommentResponseMessage.*;
...
public class MissionCommentController {
	 ...
	 */
	@PostMapping
	public ResponseEntity<SuccessResponse<CreateCommentResponse>> createMissionComment(
		@AuthenticationPrincipal User user,
		@PathVariable Long teamId,
		@PathVariable Long missionArchiveId,
		@Valid @RequestBody CreateCommentRequest createCommentRequest) {
		return ResponseEntity.ok(
			SuccessResponse.create(BoardCommentResponseMessage.CREATE_BOARD_COMMENT_SUCCESS.getMessage(),
				this.createMissionCommentUseCase.createBoardComment(user.getSocialId(), teamId, missionArchiveId,
	...
	public ResponseEntity<SuccessResponse> deleteMissionComment(@AuthenticationPrincipal User user,
		...
		return ResponseEntity.ok(
			SuccessResponse.create(BoardCommentResponseMessage.DELETE_BOARD_COMMENT_SUCCESS.getMessage()));
	}
	...
	public ResponseEntity<SuccessResponse<GetCommentResponse>> getMissionCommentAll(@AuthenticationPrincipal User user,
		@PathVariable Long teamId,
		@PathVariable Long missionArchiveId) {
		return ResponseEntity.ok(SuccessResponse.create(GET_BOARD_COMMENT_ALL_SUCCESS.getMessage(),
			this.getMissionCommentUseCase.getBoardCommentAll(user.getSocialId(), teamId, missionArchiveId)));
	}
}
