...
public class SendBoardCommentAlarmUseCase {
	...
	public void sendCommentAlarm(BaseBoardServiceResponse response, BoardComment comment) {
		...
		Optional<List<NewUploadInfo>> newUploadInfos = boardCommentGetService.getNewUploadInfo(member.getMemberId(),
			board.getBoardId());
	...
	private void sendBoardWriter(Board board, Member member, String title, String body, Team team,
		...
		if (checkBoardWriter(receiver, member, newUploadInfos)) {
			eventPublisher.publishEvent(
				new SingleFcmEvent(receiver, title, body, createIdInfo(team.getTeamId(), board.getBoardId()),
					team.getName(), AlarmType.COMMENT, PagePath.NOTICE_PATH.getValue(), receiver.isCommentPush()));
		}
	}
	...
	private void sendBoardCommentWriter(Board board, Member member, String title, String body, Team team,
		...
		eventPublisher.publishEvent(new MultiFcmEvent(title, body, memberIdAndTokensByPush, memberIdAndTokensBySave,
			createIdInfo(team.getTeamId(), board.getBoardId()), team.getName(), AlarmType.COMMENT,
			PagePath.NOTICE_PATH.getValue()));
	}
