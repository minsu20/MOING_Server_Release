...
public class MissionArchiveQueryService {
	...
	public MissionArchive findOneMyArchive(Long memberId, Long missionId, Long count) {
		...
		List<MissionArchive> missionArchives = missionArchiveRepository.findMyArchives(memberId, missionId)
			.orElseThrow(NotFoundMissionArchiveException::new);
		return missionArchives.stream()
			.filter(m -> m.getCount().equals(count))
			.findFirst()
			.orElseThrow(NotFoundMissionArchiveException::new);
	...
	public List<MissionArchive> findOthersArchive(Long memberId, Long missionId) {
		return missionArchiveRepository.findOthersArchives(memberId, missionId)
			.orElseThrow(NotFoundMissionArchiveException::new);
	}

	public Boolean isDone(Long memberId, Long missionId) {
		Optional<List<MissionArchive>> byMemberId = missionArchiveRepository.findArchivesByMissionIdAndMemberId(
			memberId, missionId);
	 ...
	 */
	public List<SingleMissionBoardRes> findMySingleMissionArchives(Long memberId, Long teamId,
		MissionStatus missionStatus) {

		// INCOMPLETE
		List<SingleMissionBoardRes> incompleteList = missionArchiveRepository.findSingleMissionInComplete(memberId,
				teamId, missionStatus, OrderCondition.DUETO)
			.orElseThrow(NotFoundMissionArchiveException::new);

		List<SingleMissionBoardRes> completeList = missionArchiveRepository.findSingleMissionComplete(memberId, teamId,
				missionStatus, OrderCondition.CREATED)
			.orElseThrow(NotFoundMissionArchiveException::new);
	...
	public List<RepeatMissionBoardRes> findMyRepeatMissionArchives(Long memberId, Long teamId,
		MissionStatus missionStatus) {
		return missionArchiveRepository.findRepeatMissionArchivesByMemberId(memberId, teamId, missionStatus)
			.orElseThrow(NotFoundMissionArchiveException::new);
	}

	public Long findDoneSingleArchives(Long missionId) {
		return missionArchiveRepository.findDonePeopleBySingleMissionId(missionId)
			.orElseThrow(NotFoundMissionArchiveException::new);
	}

	public Long findDoneRepeatArchives(Long missionId) {
		return missionArchiveRepository.findDonePeopleByRepeatMissionId(missionId)
			.orElseThrow(NotFoundMissionArchiveException::new);
	}

	public Long findMyDoneArchives(Long memberId, Long missionId) {
		return missionArchiveRepository.findMyDoneCountByMissionId(missionId, memberId)
			.orElseThrow(NotFoundMissionArchiveException::new);
	}

	public List<FinishMissionBoardRes> findMyFinishMissions(Long memberId, Long teamId) {
		return missionArchiveRepository.findFinishMissionsByStatus(memberId, teamId)
			.orElseThrow(NotFoundMissionArchiveException::new);
	}
