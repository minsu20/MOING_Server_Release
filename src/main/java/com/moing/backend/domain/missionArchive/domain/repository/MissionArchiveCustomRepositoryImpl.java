...
import static com.moing.backend.domain.teamMember.domain.entity.QTeamMember.*;
...
public class MissionArchiveCustomRepositoryImpl implements MissionArchiveCustomRepository {
	...
	@Override
	public Optional<List<SingleMissionBoardRes>> findSingleMissionInComplete(Long memberId, Long teamId,
		MissionStatus status,
		OrderCondition orderCondition) {

		BooleanExpression isReadExpression = MissionReadRepositoryUtils.isMissionReadByMemberIdAndTeamId(memberId,
			teamId);
	...
	@Override
	public Optional<List<SingleMissionBoardRes>> findSingleMissionComplete(Long memberId, Long teamId,
		MissionStatus status,
		OrderCondition orderCondition) {
		BooleanExpression isReadExpression = MissionReadRepositoryUtils.isMissionReadByMemberIdAndTeamId(memberId,
			teamId);
	...
	public Optional<List<MissionArchive>> findOthersArchives(Long memberId, Long missionId) {
		...
		BooleanExpression blockCondition = BlockRepositoryUtils.blockCondition(memberId,
			missionArchive.member.memberId);
		...
		return Optional.ofNullable(queryFactory
			...
			.where(
				...
				missionArchive.status.eq(MissionArchiveStatus.COMPLETE)
					.or(missionArchive.status.eq(MissionArchiveStatus.SKIP)),
				blockCondition,
				(missionArchive.mission.type.eq(MissionType.REPEAT)
					.and(missionArchive.mission.status.eq(MissionStatus.ONGOING)).and(dateInRange))
					.or(missionArchive.mission.type.eq(MissionType.REPEAT)
	...
	@Override
	public Optional<List<RepeatMissionBoardRes>> findRepeatMissionArchivesByMemberId(Long memberId, Long teamId,
		MissionStatus status) {
	...
	public Optional<List<FinishMissionBoardRes>> findFinishMissionsByStatus(Long memberId, Long teamId) {
		...
		Expression<String> dueToString = Expressions.stringTemplate("DATE_FORMAT({0}, '%Y-%m-%d %H:%i.%s')",
			mission.dueTo);
