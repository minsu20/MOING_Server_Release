...
public class MissionArchiveReadUseCase {
	...
	public MyMissionArchiveRes getMyArchive(String userSocialId, Long missionId) {
		...
		List<MissionArchiveRes> missionArchiveRes = MissionArchiveMapper.mapToMissionArchiveResList(
			missionArchiveQueryService.findMyArchive(memberId, missionId), memberId);
	...
	public List<PersonalArchiveRes> getPersonalArchive(String userSocialId, Long missionId) {
		...
		Long memberId = memberGetService.getMemberBySocialId(userSocialId).getMemberId();
		return MissionArchiveMapper.mapToPersonalArchiveList(
			missionArchiveQueryService.findOthersArchive(memberId, missionId), memberId);
	}
