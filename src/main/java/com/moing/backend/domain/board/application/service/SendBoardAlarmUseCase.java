...
import static com.moing.backend.global.config.fcm.constant.NewNoticeUploadMessage.*;
...
public class SendBoardAlarmUseCase {
	...
	public void sendNewUploadAlarm(BaseServiceResponse baseServiceResponse, Board board) {
		...
		if (board.isNotice()) {
			...
			Optional<List<NewUploadInfo>> newUploadInfos = teamMemberGetService.getNewUploadInfo(team.getTeamId(),
				member.getMemberId());
			...
			eventPublisher.publishEvent(new MultiFcmEvent(title, body, memberIdAndTokensByPush, memberIdAndTokensBySave,
				createIdInfo(team.getTeamId(), board.getBoardId()), team.getName(), NEW_UPLOAD,
