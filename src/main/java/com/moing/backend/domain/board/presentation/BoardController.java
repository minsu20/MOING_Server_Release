...
public class BoardController {
	...
	public ResponseEntity<SuccessResponse<CreateBoardResponse>> createBoard(@AuthenticationPrincipal User user,
		...
		return ResponseEntity.ok(SuccessResponse.create(CREATE_BOARD_SUCCESS.getMessage(),
			this.createBoardUseCase.createBoard(user.getSocialId(), teamId, createBoardRequest)));
	}
	...
	public ResponseEntity<SuccessResponse<UpdateBoardResponse>> updateBoard(@AuthenticationPrincipal User user,
		...
		return ResponseEntity.ok(SuccessResponse.create(UPDATE_BOARD_SUCCESS.getMessage(),
			this.updateBoardUseCase.updateBoard(user.getSocialId(), teamId, boardId, updateBoardRequest)));
	}
	...
	public ResponseEntity<SuccessResponse<GetBoardDetailResponse>> getBoardDetail(@AuthenticationPrincipal User user,
		@PathVariable Long teamId,
		@PathVariable Long boardId) {
		return ResponseEntity.ok(SuccessResponse.create(GET_BOARD_DETAIL_SUCCESS.getMessage(),
			this.getBoardUseCase.getBoardDetail(user.getSocialId(), teamId, boardId)));
	}
	...
	public ResponseEntity<SuccessResponse<GetAllBoardResponse>> getBoardAll(@AuthenticationPrincipal User user,
		@PathVariable Long teamId) {
		return ResponseEntity.ok(SuccessResponse.create(GET_BOARD_ALL_SUCCESS.getMessage(),
			this.getBoardUseCase.getAllBoard(user.getSocialId(), teamId)));
	}
