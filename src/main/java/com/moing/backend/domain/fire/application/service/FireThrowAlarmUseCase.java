...
import static com.moing.backend.domain.history.domain.entity.AlarmType.*;
import static com.moing.backend.domain.history.domain.entity.PagePath.*;
...
public class FireThrowAlarmUseCase {
	...
	public void sendFireThrowAlarm(Member throwMember, Member receiveMember, Team team, Mission mission,
		...
		String idInfo = createIdInfo(mission.getType() == MissionType.REPEAT, mission.getTeam().getTeamId(),
			mission.getId(), fireThrowReq == null);

		eventPublisher.publishEvent(
			new SingleFcmEvent(receiveMember, title, message, idInfo, team.getName(), FIRE, MISSION_PATH.getValue(),
				receiveMember.isFirePush()));
	}
	...
	public String getRandomMessage(String pusher, String receiver, int num) {
		...
		switch (num) {
			...
			case 1:
				return NEW_FIRE_THROW_MESSAGE2.toFrom(receiver, pusher);
