...
import static com.moing.backend.domain.fire.domain.entity.QFire.*;
			...
			select(missionArchive.member.memberId)
				...
				.where(missionArchive.mission.id.eq(missionId),
					...
					(missionArchive.mission.type.eq(MissionType.REPEAT)
						.and(dateInRange)
						.and(hasAlreadyVerifiedToday)).or(missionArchive.mission.type.eq(MissionType.ONCE))
				)
				...
				.having(
					...
					(missionArchive.mission.type.eq(MissionType.REPEAT)).or(
	...
	private JPQLQuery<Long> allMissionDone(Long missionId) {
		...
		return
			select(missionArchive.member.memberId)
				...
				.where(missionArchive.mission.id.eq(missionId),
					mission.id.eq(missionId),
					(missionArchive.mission.type.eq(MissionType.REPEAT).and(dateInRange)).or(
						missionArchive.mission.type.eq(MissionType.ONCE))
				)
				.groupBy(missionArchive.member.memberId,
					missionArchive.mission.id,
